    ...
    public class RecipeRepository : IRecipeRepository
    {
        ...
        public async Task<IEnumerable<Recipe>> FindByIngredientsAsync(string[] ingredients)
        {
            ...
            try
            {
                ...
                WHERE 
                    // Если список ингредиентов пуст – не фильтруем, иначе требуем хоть одно совпадение
                    (SIZE(available_ingredients) = 0 OR ANY(ri IN recipe_ingredients WHERE ri IN available_ingredients))
                RETURN r.title AS title, r.url AS url, r.photo_link AS photo, 
                       r.cooking_time AS cooking_time, r.instructions AS instructions,
                       recipe_ingredients AS ingredients";
                ...
                var recipes = (await result.ToListAsync()).Select(record =>
                    new Recipe
                    {
                        ...
                        // Извлекаем список шагов приготовления (ожидается список строк)
                        Instructions = record["instructions"].As<List<string>>(),
                        // Извлекаем список ингредиентов как список строк
        ...
        public async Task<IEnumerable<Recipe>> FindByIngredientsWithMissingOneAsync(string[] ingredients)
        {
            ...
            try
            {
                ...
                WHERE 
                    (SIZE(available_ingredients) = 0 OR ANY(ri IN recipe_ingredients WHERE ri IN available_ingredients))
                    AND SIZE(missing_ingredients) <= 2
                RETURN r.title AS title, r.url AS url, r.photo_link AS photo, 
                       r.cooking_time AS cooking_time, r.instructions AS instructions,
                       recipe_ingredients AS ingredients,
                ...
                var recipes = (await result.ToListAsync()).Select(record =>
                    new Recipe
                    {
                        ...
                        Photo = record["photo"].As<string>(),
                        CookingTime = record["cooking_time"].As<int>(),
                        // Извлекаем список шагов приготовления как список строк
                        Instructions = record["instructions"].As<List<string>>(),
                        // Извлекаем список ингредиентов как список строк
                        Ingredients = record["ingredients"].As<List<string>>(),
